...
exports.loadNode = exports.loadBody = void 0;
const scope_1 = require("./scope");
const modFunc_1 = require("./modFunc");
const utils_1 = require("./utils");
/** 执行代码块：并返回最后一个返回值 */
function loadBody(body, _g, resArgs = []) {
    if (!(_g instanceof Object)) {
        return;
    }
    let $$node = scope_1.getValue(_g, "$$node");
    body.forEach(node => {
        if ($$node && $$node.scope) {
            return; // 光标所在位置已找到，则退出
        }
        if ($$node && utils_1.isDownScope(node, $$node)) {
            $$node = undefined;
            _g = scope_1.newScope(_g); // 光标下方代码：使用新的作用域
        }
        switch (node.type) {
                ...
                node.clauses.forEach(n => {
                    switch (n.type) {
                        ...
                        case 'ElseifClause':
                            ...
                            loadBody(n.body, scope_1.newScope(_g), resArgs);
                            break;
                        case 'ElseClause':
                            loadBody(n.body, scope_1.newScope(_g), resArgs);
                    }
            ...
            case 'WhileStatement':
                ...
                loadBody(node.body, scope_1.newScope(_g), resArgs);
                break;
            case 'DoStatement':
                loadBody(node.body, scope_1.newScope(_g), resArgs);
                break;
...
/** 加载代码节点 */
function loadNode(node, _g) {
    if (!(_g instanceof Object)) {
        return;
    }
    // 模块文件路径
    let $file = scope_1.getValue(_g, "$file");
    // 光标所在位置
    let $$node = scope_1.getValue(_g, "$$node");
    if ($$node && $$node.scope) {
        // return; // 光标所在位置已找到，则退出
    ...
    if (node.type === "LocalStatement" || node.type === "AssignmentStatement") {
        ...
        node.variables.forEach((n, i) => {
            switch (n.type) {
                case "Identifier":
                    ...
                    scope_1.setValue(_g, n.name, res[i], isLocal, n.loc); // local变量或者upvalue
                    break;
                case "MemberExpression": {
                    ...
                    if (res[i] instanceof Object && typeof res[i]["()"] === "function") {
                        // 生成请求参数类型
                        let $$req = scope_1.getValue(_g, "$$req");
                        if ($$req && $$req[k])
                            res[i]["()"]["$$req"] = $$req[k];
                        // 生成返回值类型 v21.11.25
                        let $$res = scope_1.getValue(_g, "$$res");
                        if ($$res && $$res[k])
                            res[i]["()"]["$$res"] = $$res[k];
                    }
                    scope_1.setChild(_g, t, n.indexer, k, res[i], n.identifier.loc);
                    break;
                }
                case "IndexExpression": {
                    ...
                    scope_1.setChild(_g, t, indexer, k, res[i], n.index.loc);
                    break;
                }
                default:
                    break;
        ...
        if (node.variables.length === 1 && node.variables[0].type === "Identifier" &&
            ...
            // { } 参数补全及提示
            if ($$node && $$node.scope) {
                let type = scope_1.getType(_g, node.variables[0].name);
                if (type instanceof Object) {
                    modFunc_1.setScopeCall(type["."], $$node, _g);
                }
            }
        }
        return;
    ...
    switch (node.type) {
        ...
        case 'IfStatement':
            node.clauses.forEach(n => {
                switch (n.type) {
                    ...
                    case 'ElseifClause':
                        loadNode(n.condition, _g);
                        loadBody(n.body, scope_1.newScope(_g));
                        break;
                    case 'ElseClause':
                        loadBody(n.body, scope_1.newScope(_g));
                }
        ...
        case 'WhileStatement': // while(condition) do ... end
            loadNode(node.condition, _g);
            loadBody(node.body, scope_1.newScope(_g));
            break;
        // DO语句: do ... end
        case 'DoStatement':
            loadBody(node.body, scope_1.newScope(_g));
        ...
        case 'ForNumericStatement': {
            ...
            scope_1.setValue(newG, node.variable.name, {}, true, node.variable.loc);
        ...
        case 'ForGenericStatement': {
            ...
            if (iter) { // 迭代函数
                while (1) {
                    ...
                    node.variables.forEach((v, i) => {
                        scope_1.setValue(newG, v.name, res[i], true, v.loc);
                    });
                    loadBody(node.body, newG);
                }
            }
            ...
            if (runCount === 0) {
                node.variables.forEach((v, i) => {
                    scope_1.setValue(newG, v.name, null, true, v.loc);
                });
                loadBody(node.body, newG);
            }
            break;
        ...
        case "Identifier": {
            if (node === $$node) {
                ...
                return scope_1.getValue(_g, node.name);
            }
        }
        ...
        case "CallExpression": {
            ...
            let argt = node.arguments.map((arg, i) => {
                // 参数提示
                if (typeof funt !== "function" && (funt instanceof Object) && $$node && utils_1.isInScope(arg, $$node)) {
                    scope_1.setValue(_g, "$$call", { args: funt.args, doc: funt.doc, index: i }, false);
                }
            ...
            return modFunc_1.callFunc(funt, ...argt);
        }
        // 运行函数: func "abc"
        ...
        case "TableCallExpression": {
            ...
            if ($$node && $$node.scope && funt instanceof Object) {
                let keys = utils_1.findKeys(argt, "$$node", $$node);
                if (keys) {
                    // console.log(keys);
                    // api 请求参数 req字段
                    if (funt.$req instanceof Object) {
                        let scope = utils_1.getItem(funt.$req, keys);
                        modFunc_1.setScopeCall(scope, $$node, _g);
                        // dao 请求参数：row字段
                    }
                        ...
                        if (typeof func === "function") {
                            scope_1.setValue(_g, "$$keys", keys, true);
                            func["$$gggg"] = _g;
                        }
                    }
                }
        ...
        case "FunctionDeclaration": {
            ...
            let fun = {
                ["()"]: modFunc_1.makeFunc(node, _g),
                doc: modFunc_1.parseFuncDoc(node, _g),
                args: "(" + args.join(", ") + ")",
                "$file": $file,
                "$loc": node.loc,
            };
            // 检查变量是否在函数定义作用内
            if ($$node && utils_1.isInScope(node, $$node)) {
                scope_1.setValue(_g, "$$func", fun["()"], false);
            }
            // 第一个参数是否 self
            ...
            if (ni) {
                switch (ni.type) {
                    case "Identifier":
                        scope_1.setValue(_g, ni.name, fun, isLocal, ni.loc); // local变量或者upvalue
                        return;
                    case "MemberExpression":
                        ...
                        if ($$res && $$res[k])
                            fun["()"]["$$res"] = $$res[k];
                        scope_1.setChild(_g, t, ni.indexer, k, fun, ni.identifier.loc);
                        return;
                }
            }
            return fun;
                ...
                switch (f.type) {
                    case "TableKey": // 索引表达式 { [k] = v }
                        ...
                        if (typeof k === "string" || typeof k === "number") {
                            scope_1.setChild(_g, t, ".", k, v, f.key.loc);
                        }
                        break;
                    case "TableKeyString": // 对象表达式 { k = v }
                        scope_1.setChild(_g, t, ".", f.key.name, v, f.key.loc);
                        break;
                    case "TableValue": // 数组表达式 { a, b, c }
                        scope_1.setChild(_g, t, ".", i++, v, f.loc);
                        break;
                }
            });
            return t;
